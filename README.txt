...
Any duplicate data created is removed through a 'set' function. Once the data is 'pickled' it is then 
...
The training data is then shuffled and the Sequential ML model is created. The model spans 200 epochs, 
...
from the model file, predict what the words are and test user input against the model.
...
Other APIs currently exist that have most of the trivia data needed. It could also utilize JSON data from 
...
of matching the question to the correct answer. In terms of plans for Swish V2 I am uncertain whether I want 
to utilize API data with a larger variety of trivia answers related to just the Warriors or if I 
should expand the list of answerable basketball questions.
...
DO NOT use the word "next" in a question and please let me know when you would like to end the chat.
